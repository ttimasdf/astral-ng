...
class AstralWidgetProviderLarge : HomeWidgetProvider() {
    override fun onUpdate(
        ...
        for (appWidgetId in appWidgetIds) {
            val views = RemoteViews(context.packageName, R.layout.widget_layout_large).apply {
                ...
                val durationText = widgetData.getString("duration", "00:00")
                setTextViewText(R.id.widget_duration, durationText)

                val serverText = widgetData.getString("server_name", "No Server")
                setTextViewText(R.id.widget_server, serverText)

                val peerCount = widgetData.getInt("peer_count", 0)
                setTextViewText(R.id.widget_peers, "$peerCount peers")

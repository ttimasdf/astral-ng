...
EasyTier Enmesh deliberately does not log packet payloads or per-packet forwarding.
...
Identify the EasyTier Enmesh/TUN interface and capture a bounded reproduction:

```sh
ip -brief address
ip route
sudo tcpdump -i <interface> -s 0 -w enmesh-repro.pcap
```
...
Select the EasyTier Enmesh/TUN interface in Wireshark. Use the route commands in
...
user-approved VPN capture application. Do not add packet capture to the EasyTier Enmesh

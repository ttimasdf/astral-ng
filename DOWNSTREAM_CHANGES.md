...
Replaces the hierarchical room topology editor with a bounded force-directed
constellation that draws only routes observed from the local device. A curated
201-entry Animals & Nature / Activities palette assigns each endpoint a stable
emoji from normalized username plus virtual IP, so every observer sees the same
identity. The same optically centered identity badge appears in the Home hero,
session details, and connection setup, where it follows draft username and IP
changes. Rooms uses `graphview` only for finite, rectangle-aware placement on
topology changes; ordinary metric refreshes do not restart layout, and hidden
application states suspend graph animation. Node spacing targets roughly 3–5
peer glyphs while adapting to canvas size, with horizontal composition on
desktop and vertical composition on mobile. Pan and zoom remain bounded within
the visible canvas. Hover-inert graph children avoid the package's child-cache
invalidation bug on desktop pointer movement. Direct edges are solid; every segment through
